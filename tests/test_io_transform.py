...
from coco_pipe.io.transform import SklearnWrapper, SpatialWhitener, _rebuild_container

# --- SklearnWrapper Tests ---

def test_sklearn_wrapper_standard_scaler():
    """Test wrapping a simple scaler."""
    X = np.array([[1., 2.], [3., 4.]]) # Mean=[2, 3], Std=[1, 1]
    ...
    assert np.allclose(res.X, [[-1., -1.], [1., 1.]])
...
def test_sklearn_wrapper_validation():
    ...
    wrapper_pca = SklearnWrapper(PCA(n_components=1))
