...
def test_embedding_dataset_glob_concat(tmp_path):
    ...
    arr2 = np.ones((3,)) # shape (3,) -> treated as single observation (1, 3) -> id "subjB"
...
def test_bids_dataset_with_mocks(monkeypatch, tmp_path):
    ...
    fake_mne_bids = types.SimpleNamespace(BIDSPath=FakeBIDSPath, read_raw_bids=lambda *a, **k: None)
    ...
    monkeypatch.setattr(dataset, "load_participants_tsv", lambda root: {"01": {"age": 30}})

    def fake_read_bids_entry(bids_path, is_pre_epoched, is_evoked, mode, window_length, stride):
    ...
    assert container.coords["time"].tolist() == [0.0, 0.3333333333333333, 0.6666666666666666, 1.0]
...
def test_tabular_reshaping_errors(tmp_path):
    ...
    ds2 = dataset_mod.TabularDataset(
        p2, 
        columns_to_dims=["L1", "L2"], # L1={A,B}, L2={1,2}
        col_sep="_"
    )
...
def test_embedding_shape_mismatch(tmp_path, caplog):
    ...
    ds = dataset_mod.EmbeddingDataset(tmp_path, dims=("f",), pattern="really_bad.npy", reader=np.load)
