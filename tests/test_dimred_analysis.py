...
def test_perturbation_importance():
    ...
    scores = perturbation_importance(model, X, feature_names=[f"F{i}" for i in range(5)])
...
def test_gradient_importance_mocked():
    ...
    with patch.dict(sys.modules, {'torch': MagicMock()}):
        ...
        mock_mean.detach.return_value.cpu.return_value.numpy.return_value = expected_scores
