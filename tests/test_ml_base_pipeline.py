...
def test_cross_val_and_baseline_evaluation_classification():
    ...
    model_configs = {
        "dummy": {"estimator": DummyClassifier(strategy="most_frequent"), "params": {}}
    }
...
def test_baseline_evaluation_regression():
    ...
    y = np.arange(10)
    model_configs = {
        "dummy": {"estimator": DummyRegressor(), "params": {"constant": 0.2}}
    }
    pipe = DummyPipeline(
        X,
        y,
...
def test_feature_selection_regression():
    ...
    assert fi["feature_0"]["weighted_mean"] == pytest.approx(
        fi["feature_0"]["mean"] * freq
    )
    # fold count
...
def test_feature_selection_backward():
    ...
    out_bw = pipe.feature_selection(
        "lr", n_features=1, direction="backward", scoring="mse"
    )
...
def test_hp_search_random_and_invalid_grid():
    ...
    with pytest.raises(ValueError):
        pipe.update_model_params(
            "dummy",
            {"C": 0.5},
            update_estimator=False,
            update_config=True,
            param_type="hp_search",
        )
    ...
    assert set(res["param_frequency"].keys()) == set(
        pipe.model_configs["dummy"].param_grid.keys()
    )
...
def test_execute_method():
    X = np.vstack([np.random.randn(10, 3) - 2, np.random.randn(10, 3) + 2])
    y = np.array([0] * 10 + [1] * 10)
    ...
    # feature_selection
    r2 = pipe.execute(
        type="feature_selection", model_name="lr", n_features=2, direction="forward"
    )
    ...
    # hp_search_fs
    r4 = pipe.execute(
        type="hp_search_fs", model_name="lr", search_type="grid", n_features=2
    )
...
def test_cross_val_scaler_injection():
    ...
    model_configs = {
        "d": {"estimator": DummyClassifier(strategy="most_frequent"), "params": {}}
    }
    pipe = DummyPipeline(
        X,
        y,
