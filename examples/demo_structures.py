...
print(f"Flattened (Spatial): {flat_spatial.shape} dims={flat_spatial.dims} | Coords: {list(flat_spatial.coords.keys())}")
...
try:
    ...
    print(f"Select Y='B' -> IDs: {subset_cond.ids[:3]}... (Total {subset_cond.shape[0]})")
    ...
    print(f"2. Subject 0 Only (Wildcard): {subset_subs.ids[:2]}... (Total {subset_subs.shape[0]})")
