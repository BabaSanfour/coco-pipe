...
def download_dataset():
    ...
    if not BIDS_ROOT.exists() or not any(BIDS_ROOT.iterdir()):
        ...
        openneuro.download(dataset=DATASET_NAME, 
...
def run_dim_reduction(raw, method="PCA", n_components=2):
    ...
    if n_components in (2, 3):
        ...
        if n_components == 2:
            ...
            ax.scatter(reduced_data[:, 0], reduced_data[:, 1], reduced_data[:, 2], alpha=0.7)
...
def main():
    ...
    reduced_data = run_dim_reduction(raw, method="PCA", n_components=2)
