...
for subject_id in subjects:
    ...
    for run in runs:
        ...
        file_paths = eegbci.load_data(
            subject=subject_id, runs=[run], path="test_data/eegmmidb_raw"
        )
        ...
        write_raw_bids(
            raw, bids_path, overwrite=True, allow_preload=True, format="BrainVision"
        )
...
logger.info(
    f"Dataset download and conversion completed. BIDS dataset saved to {bids_root}"
)
