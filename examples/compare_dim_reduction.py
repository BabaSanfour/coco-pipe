...
def visualize_grid(results, y, class_names, dataset_name):
    ...
    for col, method in enumerate(methods):
        ...
        for row, (X_reduced, param_str, _) in enumerate(method_results):
            ...
            ax.scatter(
                X_reduced[:, 0], X_reduced[:, 1], c=y_int, cmap="tab10", alpha=0.5, s=1
            )
    ...
    plt.suptitle(
        f"Dimension Reduction Comparison - {dataset_name}", fontsize=18, y=0.98
    )
    plt.tight_layout()
            ...
            for params, param_str in param_sets:
                try:
                    X_reduced, elapsed_time = apply_dim_reduction(
                        X, method, n_components=n_components, **params
                    )
