...
try:
    ...
    warnings.warn(
        "pacmap not found. PaCMAP dimensionality reduction will not be available."
    )
...
try:
    ...
    print(
        "coco_pipe package not found. Please install it or add it to your PYTHONPATH."
    )
    sys.exit(1)
...
def load_preprocessed_features():
    ...
    if not features_file.exists():
        ...
        logger.info(
            "Please run scripts/process_eeg_features.py first to extract features"
        )
        return None
...
def extract_features_from_window(window_data, sfreq=160):
    ...
    for ch_idx in range(window_data.shape[0]):
        ...
        for band_name, (fmin, fmax) in FREQ_BANDS.items():
            f, psd = signal.welch(
                channel_data, fs=sfreq, nperseg=min(256, len(channel_data))
            )
        ...
        features[f"ch{ch_idx}_zero_crossing_rate"] = len(zero_crossings) / len(
            channel_data
        )
        ...
        if len(channel_data) > 1:
            ...
            sef_95_idx = (
                np.argmax(cumsum >= 0.95) if np.any(cumsum >= 0.95) else len(cumsum) - 1
            )
            sef_95 = f[sef_95_idx]
            ...
            # Spectral entropy
            psd_norm = (
                psd / np.sum(psd) if np.sum(psd) > 0 else np.ones_like(psd) / len(psd)
            )
    ...
    channel_subset = list(
        range(0, window_data.shape[0], max(1, window_data.shape[0] // 5))
    )[:5]
    ...
    for i in range(len(channel_subset)):
        for j in range(i + 1, len(channel_subset)):
            ...
            f, coh = signal.coherence(
                data_i, data_j, fs=sfreq, nperseg=min(256, len(data_i))
            )
            ...
            if np.sum(hilbert_i) > 0 and np.sum(hilbert_j) > 0:
                features[f"phase_sync_ch{ch_i}_ch{ch_j}"] = np.abs(
                    np.corrcoef(hilbert_i, hilbert_j)[0, 1]
                )
            else:
...
def process_raw_eeg_with_windows(args):
    ...
    try:
        ...
        if task_ids:
            task_names = [
                f"{tid} ({TASK_CODES.get(tid, 'unknown')})" for tid in task_ids
            ]
    ...
    if not edf_dir.exists():
        ...
        logger.info(
            "Please run scripts/process_eeg_features.py first to extract the dataset"
        )
        return None
    ...
    if subject_ids is None:
        ...
        if args.num_subjects is not None:
            ...
            logger.info(
                f"Limiting to first {args.num_subjects} subjects: {subject_ids}"
            )
        logger.info(
    ...
    for edf_file, subject_id, task_id in tqdm(
        ...
        try:
            ...
            if raw.info["sfreq"] > 120:  # Only if Nyquist > 60Hz
                raw.notch_filter(
                    np.array([60]), filter_length="auto", phase="zero", verbose=False
                )
        except Exception as e:
        ...
        try:
            ...
            if len(raw.annotations) > 0:
                ...
                events = mne.find_events(
                    raw, stim_channel="STI 014", consecutive=True, verbose=False
                )
                if len(events) == 0:
                ...
                {str(i): i for i in np.unique(events[:, 2])}
        except Exception as e:
        ...
        try:
            ...
            for event_idx, event in enumerate(events):
                ...
                # Apply moving window
                window_starts = range(
                    0, epoch_data.shape[1] - window_size_samples, window_step_samples
                )
...
def prepare_features_for_dim_reduction(df, args):
    ...
    if len(df) > args.n_samples:
        logger.info(
            f"Subsampling from {len(df)} to {args.n_samples} samples for visualization"
        )
    ...
    if "label_name" in df.columns:
        ...
        metadata["label_name"] = np.array(
            [EVENT_CODES.get(label, str(label)) for label in metadata["label"]]
        )
    ...
    if "task_name" in df.columns:
        ...
        metadata["task_name"] = np.array(
            [TASK_CODES.get(tid, str(tid)) for tid in metadata["task_id"]]
        )
...
def apply_dimensionality_reduction(X, labels, args, metadata=None):
    ...
    method_params = {
        "PCA": [({}, "Default"), ({"whiten": True}, "Whitened")],
        "TSNE": [
            (
                {"perplexity": 30, "learning_rate": "auto", "max_iter": 1000},
                "perplexity=30",
            ),
            (
                {"perplexity": 50, "learning_rate": "auto", "max_iter": 1000},
                "perplexity=50",
            ),
        ],
        "UMAP": [
    ...
    if "PACMAP" in methods and PACMAP_AVAILABLE:
        ...
        for params, param_str in pacmap_params:
            try:
                ...
                logger.info(
                    f"PaCMAP with {param_str} completed in {elapsed_time:.2f} seconds"
                )
            except Exception as e:
    ...
    for method in methods:
        ...
        for params, param_str in method_params[method]:
            try:
                ...
                elif hasattr(reducer, "model") and hasattr(
                    reducer.model, "explained_variance_ratio_"
                ):
                ...
                logger.info(
                    f"{method} with {param_str} completed in {elapsed_time:.2f} seconds"
                )
            except Exception as e:
...
def visualize_results(results, labels, label_names, metadata=None, args=None):
    ...
    if color_by == "task_label" and metadata and "task_label" in metadata:
        ...
        value_names = {}
        for i, (task_label, task_label_name) in enumerate(
            zip(
                metadata["task_label"],
                metadata.get(
                    "task_label_name", ["unknown"] * len(metadata["task_label"])
                ),
            )
        ):
        ...
        for task_id, task_name in zip(
            metadata["task_id"],
            metadata.get("task_name", ["unknown"] * len(metadata["task_id"])),
        ):
        ...
        logger.warning(
            f"Color by option '{color_by}' not available, falling back to 'label'"
        )
        color_values = labels
        unique_values = np.unique(labels)
        value_names = {label: name for label, name in zip(labels, label_names)}
    ...
    for col, method in enumerate(methods):
        ...
        for row, result in enumerate(method_results):
            ...
            ax.text(
                0.5, -0.1, param_text, ha="center", transform=ax.transAxes, fontsize=10
            )
        ...
        for method in methods:
            for result in results[method]:
                try:
                    ...
                    logger.error(
                        f"Error creating combined visualization for {method}: {e}"
                    )
            ...
            for result in results[method]:
                try:
                    ...
                    for i, value in enumerate(unique_values):
                        ...
                        label_text = (
                            labels_for_legend[i]
                            if i < len(labels_for_legend)
                            else f"{value}"
                        )
                        plt.scatter(
                            result["data"][mask, 0],
                            result["data"][mask, 1],
                            c=metadata["window_time"][mask],
                            cmap="viridis",
                            alpha=0.7,
                            s=15,
                            label=label_text,
                    ...
                    logger.error(
                        f"Error creating time-based visualization for {method}: {e}"
                    )
    else:
        logger.info(
            "Skipping time-based visualizations (window_time data not available)"
        )
    ...
    try:
        ...
        try:
            ...
            if len(df) > 10000:
                ...
                df_sample.to_csv(
                    RESULTS_DIR / "eeg_window_features_sample.csv", index=False
                )
            else:
