...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Download and prepare EEG Motor Imagery Dataset"
    )
    parser.add_argument(
        "--download-only",
        action="store_true",
...
def download_dataset():
    ...
    if DATASET_ZIP.exists():
        ...
        args = parse_arguments()
        if args.force:
            logger.info("Removing existing dataset file due to --force flag")
            DATASET_ZIP.unlink()
        else:
...
def preprocess_dataset(subject_ids=None):
    ...
    if not edf_files:
        logger.info(
            "No EDF files found in temp directory. Attempting direct file download..."
        )
        if not download_dataset_files():
            logger.warning(
                "Direct download failed. Falling back to synthetic data generation."
            )
