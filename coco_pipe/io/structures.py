...
import pandas as pd
from .utils import make_strata, sample_indices, row_quality_score
...
class DataContainer:
    ...
    def isel(self, **indexers) -> 'DataContainer':
        ...
        **indexers : dict
        ...
        current_obs_len = self.X.shape[0] if 'obs' in self.dims else 0
        ...
        for dim_name, indices in indexers.items():
            if dim_name not in self.dims:
                logger.warning(f"Dimension {dim_name} not in {self.dims}, skipping isel.")
                continue
    ...
    def balance(
        ...
        **kwargs : dict
        ...
        # 2. Get Indices
        if target not in df_meta.columns: raise ValueError(f"Target '{target}' missing")
        strategy = strategy.lower()
        counts = df_meta[target].value_counts()
        min_c, max_c = int(counts.min()), int(counts.max())
        
        if strategy == "auto":
            strategy = "undersample" if (min_c * counts.size) >= len(df_meta)/2 else "oversample"
        ...
        if not covariates:
            size = {c: min_c if strategy=="undersample" else max_c for c in counts.index}
            indices_val = sample_indices(df_meta, target, size, rng, strategy!="undersample", kwargs.get('prefer_clean_rows', False), exclude)
        
        else:
            # 2. Covariate Balancing (Stratified)
            # kwargs.get('n_bins', 5), kwargs.get('binning', 'quantile')
            strata_s = make_strata(df_meta, covariates, kwargs.get('n_bins', 5), kwargs.get('binning', 'quantile'))
            ...
            for _, g in tmp.groupby("__strata__"):
                ...
                if len(sc) <= 1:
                    ...
                    if strategy != "undersample":
                         ...
                         indices_parts.append(sample_indices(g, target, size_map, rng, True, kwargs.get('prefer_clean_rows', False), exclude))
                    continue
                
                # Balance locally within stratum
                sz = {c: int(sc.min()) if strategy=="undersample" else int(sc.max()) for c in sc.index}
                indices_parts.append(sample_indices(g, target, sz, rng, strategy!="undersample", kwargs.get('prefer_clean_rows', False), exclude))
                    
            if not indices_parts:
                # Fallback: global balance
                sz = {c: min_c if strategy=="undersample" else max_c for c in counts.index}
                indices_val = sample_indices(df_meta, target, sz, rng, strategy!="undersample", kwargs.get('prefer_clean_rows', False), exclude)
            else:
                combined = pd.concat([pd.Series(i) for i in indices_parts]).sample(frac=1.0, random_state=rng)
    ...
    def select(self, ignore_case: bool = False, fuzzy: bool = False, **selections) -> 'DataContainer':
        """
        ...
        for key, query in selections.items():
            ...
            if key == 'y' and self.y is not None:
                ...
                if matched_dim:
                    ...
                    logger.warning(f"Aux coordinate '{key}' len={len(target_arr)} matches no dimension shape {self.X.shape}. Ignoring selection.")
                    continue
            
            if target_arr is None:
                 if key in self.dims and key not in self.coords:
                     logger.warning(f"Selection on dim '{key}' ignored (no coordinates).")
                     continue
                 logger.warning(f"Selection key '{key}' not found in logical dims, y, ids, or aux coords. Ignoring.")
                 continue
            ...
            if len(target_arr) == 0:
                 logger.warning(f"Target array for '{key}' is empty. Skipping selection.")
                 continue
            ...
            if callable(query):
                ...
                if not isinstance(mask, (np.ndarray, list)) or len(mask) != len(target_arr):
                     raise ValueError(f"Callable query for '{key}' must return boolean array of shape {target_arr.shape}.")
                ...
                for op, val in query.items():
                    if op not in ops:
                        raise ValueError(f"Unknown operator '{op}'. Supported: {list(ops.keys())}")
                ...
                if is_str_target:
                    ...
                    if fuzzy:
                        for q in query_list_proc:
                            matches = difflib.get_close_matches(q, target_lookup, n=3, cutoff=0.6)
            ...
            if len(indices) == 0:
                 raise ValueError(f"Selection for '{key}' resulted in empty set. Query: {query}")
    ...
    def flatten(self, preserve: Union[str, List[str]] = "obs") -> 'DataContainer':
        ...
        # Current indices
        permute_order = [self.dims.index(p) for p in preserve] + [self.dims.index(d) for d in to_flatten]
        ...
        flattened_len = int(np.prod([self.X.shape[self.dims.index(d)] for d in to_flatten]))
        ...
        if flat_coords_list:
            ...
            if total_size < 200000: # Limit to ~200k features strings
                combo_labels = ["_".join(map(str, x)) for x in itertools.product(*flat_coords_list)]
    ...
    def aggregate(self, by: Union[str, np.ndarray, List[Any]], method: str = 'mean') -> 'DataContainer':
        ...
        if len(groups) != n_obs:
            raise ValueError(f"Grouping array length {len(groups)} must match obs length {n_obs}.")
        ...
        keys_to_drop = []
        for k, v in self.coords.items():
            if isinstance(by, str) and k == by: continue 
            if k == 'obs': continue
        ...
        return replace(
            ...
            coords=new_coords,
            meta={**self.meta, 'aggregated': True, 'agg_by': str(by), 'agg_method': method}
        )
