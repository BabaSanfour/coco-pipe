...
def register_backend(
    name: str,
    backend_cls: type[BackendBase],
    *,
    overwrite: bool = False,
) -> None:
    """Register a foundation backend for plugins and deterministic test doubles."""
    if name in _BACKEND_MAP and not overwrite:
        raise KeyError(f"Foundation backend '{name}' is already registered.")
    if not issubclass(backend_cls, BackendBase):
        raise TypeError("backend_cls must inherit BackendBase.")
    _BACKEND_MAP[name] = backend_cls


def unregister_backend(name: str) -> None:
    """Remove a dynamically registered backend."""
    if name in {"hugging_face", "braindecode"}:
        raise ValueError(f"Built-in backend '{name}' cannot be unregistered.")
    _BACKEND_MAP.pop(name, None)
...
def load(
    ...
    **backend_kwargs,
    ...
    signal_metadata : SignalMetadata, optional
        Input-data ``sfreq`` and ``ch_names``. Declare it here so the returned
        backend is immediately ready for ``transform`` / ``predict`` / ``fit``
        with no separate binding step. (This is *input* metadata, distinct from
        the model's pretrained expectations in the registry spec.)
    **backend_kwargs
    ...
    if signal_metadata is not None:
        backend_obj.signal_metadata_ = signal_metadata
    return backend_obj
